...
describe("config.vsCodeVersion", () => {
	it("should consider scrollable hovers for >= code 1.6", () => {
		...
		assert.equal(new config.CodeCapabilities("1.6.0-dev").hasScrollableHovers, false);
		assert.equal(new config.CodeCapabilities("1.6.0").hasScrollableHovers, true);
		assert.equal(new config.CodeCapabilities("1.6.1-dev").hasScrollableHovers, true);
