...
describe("file tracker", () => {
	...
	it("includes visible editors in the priority list", async function () {
		if (!extApi.dasFileTracker.getLastPriorityFiles) {
			this.skip();
			return;
		}

		await closeAllOpenFiles();
		assert.deepStrictEqual(extApi.dasFileTracker.getLastPriorityFiles(), []);
		await openFile(file1);
		assert.deepStrictEqual(extApi.dasFileTracker.getLastPriorityFiles(), [fsPath(file1)]);
	});
	...
	it("excludes open but not-visible editors from the priority list", async function () {
		...
		assert.deepStrictEqual(extApi.dasFileTracker.getLastPriorityFiles(), [fsPath(file2)]);
	});
	...
	describe("subscriptions", function () {
		...
		it("includes visible editors", async function () {
			if (!extApi.dasFileTracker.getLastSubscribedFiles) {
				this.skip();
				return;
			}

			await openFile(file1);
			assert.deepStrictEqual(extApi.dasFileTracker.getLastSubscribedFiles(), [fsPath(file1)]);
		});
		...
		it("includes open but not-visible editors", async function () {
			...
			await openFile(file2);
			assert.deepStrictEqual(extApi.dasFileTracker.getLastSubscribedFiles(), [fsPath(file1), fsPath(file2)]);
		});
		...
		it("exclude closed editors", async function () {
			...
			await closeFile(file1);
			await waitForResult(() => extApi.dasFileTracker.getLastSubscribedFiles!().length === 0, "Closed file was not removed from subscription list", threeMinutesInMilliseconds);
		});

		it("tracks outlines for open files", async () => {
			await waitForResult(() => !extApi.dasFileTracker.getOutlineFor(outlineTrackingFile), "Outline was already present");
			await openFile(outlineTrackingFile);
			await waitForResult(() => !!extApi.dasFileTracker.getOutlineFor(outlineTrackingFile), "Outline was not added");
		});

		it("removes tracked outlines when files are closed", async () => {
			// Ensure the outline is present first, else the test is invalid.
			await openFile(outlineTrackingFile);
			await waitForResult(() => !!extApi.dasFileTracker.getOutlineFor(outlineTrackingFile), "Outline was never present");

			// Close the file and ensure it disappears within the expected timeframe (3 minutes!!).
			await closeAllOpenFiles();
			await waitForResult(() => !extApi.dasFileTracker.getOutlineFor(outlineTrackingFile), "Outline was not removed", threeMinutesInMilliseconds);
		});
	});
});
