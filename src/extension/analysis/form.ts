...
/**
 * InteractiveMiddleware defines optional middleware hooks for intercepting
 * custom interactive refactoring LSP requests.
 */
export interface InteractiveMiddleware extends Middleware {
	interactiveExecuteCommand?: InteractiveExecuteCommandMiddleware;

	resolveCommand?: InteractiveResolveCommandMiddleware;

	// TODO(hxjiang): is that really necessary to make this middleware.
	interactiveListEnum?: InteractiveListEnumMiddleware;
}
...
export interface InteractiveListEnumMiddleware {
	(this: void, param: InteractiveListEnumParams, next: InteractiveListEnumSignature): vscode.ProviderResult<
		FormEnumEntry[]
	>;
}

/**
 * InteractiveLanguageClient extends LanguageClient with typed clientOptions.
 */
export class InteractiveLanguageClient extends LanguageClient {
	// TODO(hxjiang): support vscode.LanguageClient contructor w/o "id".mail
	constructor(
		id: string,
		name: string,
		serverOptions: ServerOptions,
		clientOptions: LanguageClientOptions | InteractiveLanguageClientOptions,
		forceDebug?: boolean
	) {
		super(id, name, serverOptions, clientOptions, forceDebug);

		const interactiveOptions = this.clientOptions;
		const middleware = interactiveOptions.middleware || {};

		interactiveOptions.middleware = {
			...middleware,
			executeCommand: async (cmd: string, args: any[], next: ExecuteCommandSignature) => {
				...
				if (!Array.isArray(supported) || !supported.includes('command')) {
					return middleware.executeCommand ? middleware.executeCommand(cmd, args, next) : next(cmd, args);
				}
				...
				// The command does not require any user input.
				if (formAnswers === undefined || formAnswers.length === 0) {
					return middleware.executeCommand ? middleware.executeCommand(cmd, args, next) : next(cmd, args);
				} else {
					const next: InteractiveExecuteCommandSignature = (cmd: string, args: any[], answers: any[]) => {
						const requestType = new RequestType<InteractiveExecuteCommandParams, any, void>(
					...
					return middleware.interactiveExecuteCommand
						? middleware.interactiveExecuteCommand(cmd, args, formAnswers, next)
						: next(cmd, args, formAnswers);
				}
			}
		};
	}

	public get clientOptions(): InteractiveLanguageClientOptions {
		return super.clientOptions as InteractiveLanguageClientOptions;
	}
	...
	public async resolveCommand(
		...
		const next: InteractiveResolveCommandSignature = (param: InteractiveExecuteCommandParams) => {
			const requestType = new RequestType<InteractiveExecuteCommandParams, InteractiveExecuteCommandParams, void>(
				'command/resolve'
			);
			return this.sendRequest(requestType, param).then(undefined, (error) => {
				return this.handleFailedRequest(requestType, undefined, error, undefined);
			});
		};
		...
		for (let i = 0; i < MAX_RETRY; i++) {
			const response = middleware?.resolveCommand
				? await middleware.resolveCommand(param, next)
				: await next(param);
			if (!response) {
				return undefined;
			}
		...
		return new Promise((resolve) => {
			...
			const search = async (query: string) => {
				...
				try {
					...
					const middleware = this.clientOptions?.middleware;
					const next = (p: InteractiveListEnumParams) => {
						const requestType = new RequestType<InteractiveListEnumParams, FormEnumEntry[], void>(
							'interactive/listEnum'
						);
						return this.sendRequest<FormEnumEntry[]>('interactive/listEnum', p).then(undefined, (error) => {
							return this.handleFailedRequest(requestType, undefined, error, undefined);
						});
					};

					const response = middleware?.interactiveListEnum
						? await middleware.interactiveListEnum(params, next)
						: await next(params);
					if (!response) {
						quickPick.items = [];
						return;
