...
export class DebugConfigProvider implements DebugConfigurationProvider {
	...
	public resolveDebugConfiguration(_folder: WorkspaceFolder | undefined, debugConfig: DebugConfiguration, _token?: CancellationToken): ProviderResult<DebugConfiguration> {
	...
	protected async buildToolArgs(debuggerType: DebuggerType, debugConfig: DartLaunchArgs, conf: ResourceConfig, _portFromLocalExtension?: number): Promise<string[]> {
		let args: string[] = [];
	...
	protected async buildFlutterToolArgs(debugConfig: DartVsCodeLaunchArgs, conf: ResourceConfig): Promise<string[]> {
		...
		if (config.shareDevToolsWithFlutter && !args.includes("--devtools-server-address")) {
			...
			try {
				...
				this.logger.error("Failed to get DevTools server address");
			}
		}
