...
export function isTestFile(file: string): boolean {
	...
	return isDartFile(file) && (isInsideFolderNamed(file, "test") || (file && file.toLowerCase().endsWith("_test.dart")));
}

export function isTestFolder(path: string): boolean {
	return isInsideFolderNamed(path, "test") && fs.existsSync(path) && fs.statSync(path).isDirectory();
}
...
export function isDartFile(file: string) {
	return path.extname(file.toLowerCase()) === ".dart" && fs.existsSync(file) && fs.statSync(file).isFile();
}
